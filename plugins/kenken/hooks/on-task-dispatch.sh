...
# ---------------------------------------------------------------------------
# 3b. Guard: only process kenken state (ignore other plugins' workflows)
# ---------------------------------------------------------------------------
...
#    Phase 2.1 is the only batch phase in kenken (implementation tasks)
# ---------------------------------------------------------------------------
...
jq -n \
  ...
  '{
    "hookSpecificOutput": {
      ...
      "additionalContext": ("Current workflow phase is " + $phase + ". Ensure this subagent is executing phase " + $phase + " work. Write output to .agents/tmp/kenken/phases/" + $output + ".")
    }
  }'
