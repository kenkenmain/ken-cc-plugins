...
Write your JSON review to `.agents/tmp/phases/loop-{{LOOP}}/A3-review.sentinel-security.json`.
...
```
[SEC-001] critical in {file_path}:{line} -- {brief description}. Suggestion: {suggestion}
[SEC-002] warning in {file_path}:{line} -- {brief description}. Suggestion: {suggestion}
```
