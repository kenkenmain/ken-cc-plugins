...
Write your JSON review to `.agents/tmp/phases/loop-{{LOOP}}/A3-review.sentinel-style.json`.
...
```
[STYLE-001] critical in {file_path}:{line} -- {brief description}. Suggestion: {suggestion}
[STYLE-002] warning in {file_path}:{line} -- {brief description}. Suggestion: {suggestion}
```
