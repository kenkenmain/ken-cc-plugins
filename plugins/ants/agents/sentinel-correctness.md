...
Write your JSON review to `.agents/tmp/phases/loop-{{LOOP}}/A3-review.sentinel-correctness.json`.
...
```
[CORR-001] critical in {file_path}:{line} -- {brief description}. Suggestion: {suggestion}
[CORR-002] warning in {file_path}:{line} -- {brief description}. Suggestion: {suggestion}
```
...
```
Sentinel correctness review complete. Found [N critical], [N warning], [N info] issues. Review at .agents/tmp/phases/loop-{{LOOP}}/A3-review.sentinel-correctness.json
```
