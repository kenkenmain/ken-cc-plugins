...
- `.agents/tmp/phases/loop-{{LOOP}}/A4-sync.json` exists with `verdict: "ship"`
...
2. Read sync summary from `.agents/tmp/phases/loop-{{LOOP}}/A4-sync.json`
...
- .agents/tmp/phases/loop-{{LOOP}}/A4-sync.json (sync summary)
...
- .agents/tmp/phases/loop-{{LOOP}}/A4-sync.json (ship verdict)
