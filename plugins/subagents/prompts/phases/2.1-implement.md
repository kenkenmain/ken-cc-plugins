...
```json
{"taskId":"task-N","description":"...","targetFiles":[...],"instructions":"...","dependencyOutputs":[...],"constraints":{"maxReadFiles":10,"maxWriteFiles":5,"allowBashCommands":false}}
```

Task agents are expected to write unit tests alongside their implementation. The `testsWritten` array in each task's output tracks what tests were produced. The `maxWriteFiles` limit (5) includes both implementation and test files. Tasks that cannot write meaningful tests (e.g., configuration-only changes) should return an empty `testsWritten: []`.
