...
- **Type:** complexity-routed task agents (wave-based parallel batch)
  ...
  - Hard: `codex-task-agent` (thin codex-high MCP wrapper)
...
| Hard   | 4+ files, >200 LOC, security | codex-task-agent    | codex-high MCP wrapper             |

For each task, pick `subagent_type` based on complexity score:
- Easy → `subagents:sonnet-task-agent`
- Medium → `subagents:opus-task-agent`
- Hard → `subagents:codex-task-agent` (if `codexAvailable: true` in state) or `subagents:opus-task-agent` (if `codexAvailable: false`)

Check `state.codexAvailable` in `.agents/tmp/state.json` to determine hard task routing.

### Task Agent Payload (all levels)
