...
3. Dispatch task analysis to Codex MCP:
   ```
...
4. Build the schedule array based on analysis and flags:
   ...
   - If `--no-test` flag or Codex says tests unnecessary: remove phases 3.1, 3.2, 3.3
...
If Codex MCP call fails during task analysis, fall back to simple heuristic analysis based on task description length and keywords. Always complete initialization — never leave state partially written.
