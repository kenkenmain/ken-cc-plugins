...
Dispatch `subagents:init-claude` with task description, ownerPpid, `codexMode: false`, and parsed flags (including `--worktree` if set). The init agent creates the worktree (only if `--worktree`), analyzes the task, and writes initial state with **Claude-only defaults**:
