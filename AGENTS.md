...
- Source plugin-local libs from `$SCRIPT_DIR/lib/`; source cross-plugin bootstrap from `plugins/shared/lib/common-state.sh` (provides flock/mkdir lock probe, macOS/Linux `stat` compatibility)

## Workflow Learnings

<!-- Auto-generated by retrospective-analyst. Do not remove this section. -->

- [2026-03-06] Task "Create ants plugin": Parallel implementation waves cause naming/reference mismatches across independently-built components (S6 review caught 10 critical issues: agent name routing, state field names, file path references, verdict value inconsistencies). Future plugin creation tasks should include a shared naming contract in the plan that all parallel tasks reference.
- [2026-03-06] Task "Create ants plugin": Plan review (S3) warnings about consolidating duplicate libraries (schedule.sh vs swarm.sh) and skipping redundant tasks prevented wasted implementation effort. Thorough plan review pays off when the plan has 40+ tasks.
- [2026-03-06] Task "Create ants plugin": Single-wave parallel dispatch of 8 tasks completed with 0 failures and 0 stage restarts -- plugin scaffolding tasks (manifest, agents, hooks, skills, commands) are highly parallelizable since they touch disjoint files.
- [2026-03-06] Task "Create ants plugin": For Markdown+shell plugins with no runtime code, S7 validation (JSON schema, bash -n, frontmatter checks, hook reference integrity) is sufficient -- S9 test development correctly identified no unit test framework was applicable and deferred to S7 coverage.
- [2026-03-06] Task "Upgrade ants v0.2": Field name mismatches between prompt templates and hook scripts are a recurring cross-component bug (S6 caught allFilesChanged vs files_changed, overallStatus vs all_complete). When prompts feed data to hooks via state.json, define the exact field names in the plan as a shared contract.
- [2026-03-06] Task "Upgrade ants v0.2": Serializing hook modifications (10 tasks touching shared files sequentially) eliminated the merge conflict problem from v0.1 where 4-6 parallel areas modified the same files. For upgrade tasks that modify existing shared files, serialize; for new-file creation, parallelize.
- [2026-03-06] Task "Upgrade ants v0.2": S3 plan review consolidation from 52 to ~30 tasks and eliminating Area 6 duplication prevented the file contention issues that plagued v0.1. Aggressive plan pruning before implementation saves more time than fixing conflicts after.
- [2026-03-06] Task "Upgrade ants v0.2": Agent tool allowlists are easy to get wrong -- S13 caught sentinels missing Write tool (needed for their core function of writing marker files). Review agents should verify that each agent's tools list matches its described responsibilities.
- [2026-03-06] Task "Upgrade ants v0.2": Loop-back and circuit breaker integration points (reset_phases_for_loop, cb_increment_stage_restarts) were planned but not wired in during implementation. Multi-component control flow paths need explicit integration test tasks in the plan, not just unit-level file checks.
